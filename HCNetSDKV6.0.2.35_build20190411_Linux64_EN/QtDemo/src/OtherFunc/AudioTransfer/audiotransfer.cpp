...
int wave_get_samples (wave_info_t *wave_info, unsigned char pcm[], int numSamples)
{
	int samples_read;
	FILE *file = wave_info->soundfile;
	
	samples_read = fread ((short int*)pcm, sizeof(short int), numSamples, file);
	...
	{
		SwapBytesInWords ((short *)pcm, samples_read);
	}
    ...
    : QDialog(parent)
{
    ...
    for ( it = (*m_qlistdevicetree).begin(); it != (*m_qlistdevicetree).end(); ++it)
    {
        if ((*it).getUsrID()>=0)
        {
            m_atuserid[i]=(*it).getUsrID();//i��indexһһ��Ӧ
            i++;
...
{
	NET_DVR_AUDIOENC_INFO strAudio ={0};
	m_encodehandle = NET_DVR_InitG722Encoder(&strAudio);
	...
	if ((para->channels!=1)&&(para->samplesize!=16)&&(para->samplerate!=16000))
	{
		...
		QMessageBox::information(this,tr("��Ƶ�ļ�����"),
			tr("��Ƶ�ļ���������"));
		return ;
	}
	...
	//{
		...
		enc_proc_param.in_buf   = (unsigned char *)encode_input;  
		enc_proc_param.out_buf  = (unsigned char *)encoded_data; 
		i=wave_get_samples(para,enc_proc_param.in_buf,strAudio.in_frame_size);

		if (!NET_DVR_EncodeG722Frame(m_encodehandle, &enc_proc_param))
		{
		...
		if (!NET_DVR_VoiceComSendData(m_talkmrhandle,(char*)enc_proc_param.out_buf,enc_proc_param.out_frame_size))
		{
