...
void Deploy::on_pushButton_deployornot_clicked()
{
    ...
    {	
       ...
       m_dphandle = NET_DVR_SetupAlarmChan_V50(m_useridtmp, &struSetupAlarmParam, NULL, 0);


        if (-1 == m_dphandle)
        {
            QMessageBox::information(this,tr("NET_DVR_SetupAlarmChan_V50 failed"), \
                    tr("SDK_Last_Error =%1 ").arg(NET_DVR_GetLastError()));
            return ;
        }
        QMessageBox::information(this,tr("NET_DVR_SetupAlarmChan_V50 SUCCESS"), 
		tr("NET_DVR_SetupAlarmChan_V50 SUCCESS"));  
